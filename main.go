...
func buildPraRequest(log *logger.Log) {
	log.WriteLog("build prarequest", logger.LogInfo, token)
	log.WriteLog("build prarequest", logger.LogInfo, token)
	log.WriteLog("build prarequest", logger.LogInfo, token)
	log.WriteLog("build prarequest", logger.LogInfo, token)
	log.WriteLog("build prarequest", logger.LogInfo, token)
...
func buildPraResponse(log *logger.Log) {
	log.WriteLog("build praresponse", logger.LogInfo, token)
	log.WriteLog("build praresponse", logger.LogInfo, token)
	log.WriteLog("build praresponse", logger.LogInfo, token)
	log.WriteLog("build praresponse", logger.LogInfo, token)
	log.WriteLog("build praresponse", logger.LogInfo, token)
...
func buildRequest(log *logger.Log) {
	httpHeader := createDynamicDummyRequestHeader()

	// getHeaderKeysAsBytes(httpHeader)

	headerBytes := getHeaderKeysAsBytes(httpHeader)
	log.WriteLog("build request", logger.LogInfo, string(headerBytes))

	time.Sleep(40 * time.Millisecond)
}

func buildResponse(logs *logger.Log) string {
	...
	logs.WriteLog("build response", logger.LogInfo, string(respJson))
